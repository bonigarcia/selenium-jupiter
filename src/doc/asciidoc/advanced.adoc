...
The use of https://www.genymotion.com/[Genymotion] for using Android devices in the cloud is possible as of Selenium-Jupiter 3.1.0. There are two options to use Genymotion from Selenium-Jupiter:

* As Software as a Service (SaaS): This feature is enabled by selecting `GENYMOTION_SAAS` in the `cloud` parameter of `@DockerBrowser`. To use this option, we need a valid Genymotion account. This account will be configured using the methods `setAndroidGenymotionUser()`, `setAndroidGenymotionPassword()` and `setAndroidGenymotionLicense()` for user, password, and license respectively. Moreoever, we need to specify the name of the Genymotion template to be used in our test using the configuration method `setAndroidGenymotionTemplate()`.

* As Platform as a Service (PaaS): This feature is enabled by selecting `GENYMOTION_PAAS` in the `cloud` parameter of `@DockerBrowser`. So far, Amazon EC2 instances are used in this mode. For this reason, the path of the configuration file `aws.json` needs to be specified using the method `setAndroidGenymotionAwsJson()`.

The following test shows an example of both usages:
...
* Using _Selenium-Jupiter_ as a https://github.com/bonigarcia/selenium-jupiter/releases/download/selenium-jupiter-3.1.0/selenium-jupiter-3.2.0-fat.jar[fat-jar]. This jar can be created using the command `mvn compile assembly:single` from the source code, and then `java -jar selenium-jupiter.jar browserName <version>`. For instance:

[source]
----
$ java -jar selenium-jupiter-3.1.0-fat.jar firefox
[INFO] Using SeleniumJupiter to execute firefox (latest) in Docker
[INFO] Using FIREFOX version 64.0 (latest)
...
* Using Selenium-Jupiter as a https://github.com/bonigarcia/selenium-jupiter/releases/download/selenium-jupiter-3.1.0/selenium-jupiter-3.2.0-fat.jar[fat-jar]. For instance:
