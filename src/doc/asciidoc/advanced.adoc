...
* Using _Selenium-Jupiter_ as a https://github.com/bonigarcia/selenium-jupiter/releases/download/selenium-jupiter-3.3.3/selenium-jupiter-3.3.3-fat.jar[fat-jar]. This jar can be created using the command `mvn compile assembly:single` from the source code, and then `java -jar selenium-jupiter.jar browserName <version>`. For instance:

[source]
----
$ java -jar selenium-jupiter-3.3.3-fat.jar firefox
...
* Using Selenium-Jupiter as a https://github.com/bonigarcia/selenium-jupiter/releases/download/selenium-jupiter-3.3.3/selenium-jupiter-3.3.3-fat.jar[fat-jar]. For instance:

[source]
----
> java -jar webdrivermanager-3.3.3-fat.jar server
[INFO] Selenium-Jupiter server listening on http://localhost:4042/wd/hub
----
