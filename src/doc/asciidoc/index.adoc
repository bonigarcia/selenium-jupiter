...
When we run the template test using this browser scenario, we will execute four tests using Chrome in Docker: the first using the stable (_latest_) version, the second using the beta version (`beta`), and the last one using the development version (`dev`). If we run the test in Eclipse, for example, we will get the following output:
...
* García, Boni, et al., "https://www.sciencedirect.com/science/article/pii/S0950584926000662[Test automation with selenium: A survey]." Information and Software Technology 194 (2026): 108077.
...
== About
Selenium-Jupiter (Copyright © 2017-2026) is an open-source project created and maintained by https://bonigarcia.dev/[Boni García] (https://twitter.com/boni_gg[@boni_gg]), licensed under the terms of https://www.apache.org/licenses/LICENSE-2.0[Apache 2.0 License]. This documentation (also available in link:selenium-jupiter.pdf[PDF]) is released under the terms of https://creativecommons.org/licenses/by-nc-sa/2.0/[CC BY-NC-SA 2.0].
