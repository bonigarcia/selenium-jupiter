...
|`@Options`|Field-level|To configure browsre options (e.g. `ChromeOptions` for Chrome, `FirefoxOptions` for Firefox, `EdgeOptions` for Edge, `OperaOptions` for Opera, and `SafariOptions` for Safari)
...
Notice that instead of declaring the method with the usual `@Test` annotation, we use the JUnit 5's annotation `@TestTemplate`. Then, the parameter type of the test template method is `WebDriver`. This type is the generic interface of Selenium WebDriver, and the concise type (i.e. `ChromeDriver,` `FirefoxDriver,` `RemoteWebDriver,` etc.) is determined by Selenium-Jupiter in runtime.
...
WARNING: Notice that the previous example deletes the image at the end of the test. If you want to keep the picture, omit the line `imageFile.delete();`.

=== Integration with Jenkins
Selenium-Jupiter seamlessly integrates with Jenkins through one of its plugins: the https://plugins.jenkins.io/junit-attachments/[Jenkins attachment plugin]. The aim is to attach output files (typically PNG screenshots and MP4 recordings of Docker browsers) and keep these files associated with job execution. This is done in Selenium-Jupiter setting the configuration key `sel.jup.output.folder` to a particular value: `surefire-reports` (see <<advanced-configuration,advanced configuration>>>). This way, Selenium-Jupiter stores the generated files in the proper folder so that the Jenkins attachment plugin can find those files and export them in the Jenkins GUI. For instance, consider the following test:

[source,java]
----
include::../../test/java/io/github/bonigarcia/seljup/test/screenshot/ScreenshotSurefireTest.java[tags=snippet-in-doc,indent=0]
----

If we run this test in Jenkins in which is already installed the attachment plugin, at the end of the execution, a recording in MP4 and a PNG screenshot are attached to the Jenkins job, as shown in the following picture:
...
|`setBrowserTemplateJsonFile` `(String)`|`sel.jup.browser.template.` `json.file`|`classpath:browsers.json`|Location of the JSON browser scenario. If this value do not start with the word `classpath:`, the JSON file will be searched using the specified path 
