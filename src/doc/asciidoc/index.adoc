...
==== Remote Desktop
A possible inconvenience of using browsers in Docker is that we cannot see what is happening inside the container by default. To improve this situation, Selenium-Jupiter allows connecting to the remote desktop session simply setting to `true` the attribute `vnc` of the `@DockerBrowser` annotation. When using this option, two different technologies are used internally:
